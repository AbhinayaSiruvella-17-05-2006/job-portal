...
function StudentDashboard() {
  ...
  useEffect(() => {
    if (email) {
      ...
      axios.get(`http://localhost:5000/api/applications/${email}`)
        .then((res) => {
          const apps = res.data;
          setStats({
            applied: apps.length,
  ...
  const handleImageUpload = (e) => {
    ...
    axios.post("http://localhost:5000/api/student/upload-pic", formData)
      .then(res => {
      ...
      <header style={styles.topNav}>
        <div style={styles.userInfoTop}>
          ...
          <div style={styles.profileCircle} onClick={() => document.getElementById("picInput").click()}>
            {profilePic ? (
              <img src={`http://localhost:5000/${profilePic}`} alt="Profile" style={styles.imageStyle} />
            ) : "👤"}
