...
function RecruiterProfile() {
  ...
  const handleUpdate = async () => {
    try {
      const res = await axios.put(`https://job-portal-xwkz.onrender.com/api/recruiter/update/${user.email}`, formData);
  ...
  const handleDelete = async () => {
    if (window.confirm("WARNING: This will permanently delete your account and all posted jobs. Continue?")) {
      try {
        await axios.delete(`https://job-portal-xwkz.onrender.com/api/recruiter/delete/${user.email}`);
        handleLogout();
      } catch (err) {
